...
public class BroadcastScheduler<T extends AnimeNightMeta> {
    ...
    public List<T> calibrate(Iterable<Anime> animes) {
        ...
        for (Anime anime : animes) {
            ...
            // Even if the ZoneID is wrong there, it's almost impossible that it is before "now"... right ?
            ...
            while (optionalFirst.isPresent()) {
                ...
                if (instance.getAmount() != correctAmount) {
                    // We wouldn't be there if you followed the schedule... Yeah, I already said that line 238...
                ...
                // But, what about first episode > anime total ?
                // - This is handled in the UpdateBroadcastTask, as this only calibrate, not removing overflows.
    ...
    public List<T> delay(TemporalAmount duration, Predicate<ZonedDateTime> predicate) {

        LOGGER.info("Trying to delay schedule by {}", duration.toString());
        ...
        for (T event : delayable) {
            ...
            // Apply delay
            night.setStartDateTime(night.getStartDateTime().plus(duration));
        ...
        for (T event : delayable) {
            // Apply delay
            event.setStartDateTime(event.getStartDateTime().plus(duration));
            event.setEndDateTime(event.getEndDateTime().plus(duration));
        }
        return delayable;
    }
    ...
    public List<T> delay(long value, TimeUnit unit, Predicate<ZonedDateTime> predicate) {
        ...
        Duration duration = Duration.of(value, unit.toChronoUnit());
        return this.delay(duration, predicate);
    }
